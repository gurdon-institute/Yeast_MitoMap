...
public class MitoMap implements Command,ActionListener{
	...
	private void mapMitochondria(){
		...
		double sphereSA = 4d*Math.PI*Math.pow(sphereR,2);	//surface area of a sphere with object volume */
			...
			if(do3D){
				...
				Content cont = univ.addVoltex(map,new org.scijava.vecmath.Color3f(java.awt.Color.WHITE),title,1,true3,1);
				univ.show();
			}
			map.changes = false;
			map.close();
	...
	public static void main(String[] arg){
		...
		ImageJ.main(arg);
		ImagePlus img = new ImagePlus("E:\\Zengyi-mitomap\\14.tif");
		img.show();
